...
const retrieveGrantFileMock = retrieveGrantFile as MockedFunction<typeof retrieveGrantFile>;
...
describe('usePermissions', () => {
  ...
  const mockPermissions: GrantedPermission[] = [
    {
      id: BigInt(1),
  ...
  describe('initialization', () => {
    ...
    it('loads user permissions automatically when vana and address are available', async () => {
      ...
      expect(mockVana.permissions.getUserPermissions).toHaveBeenCalledWith({ limit: 20 });
  ...
  describe('loadUserPermissions', () => {
    ...
    it('handles errors when loading permissions fails', async () => {
      ...
      mockVana.permissions.getUserPermissions.mockRejectedValue(new Error('Network error'));
      ...
      expect(result.current.userPermissions).toEqual([]);
      expect(consoleSpy).toHaveBeenCalledWith('Failed to load user permissions:', expect.any(Error));
  ...
  describe('handleGrantPermission', () => {
    ...
    it('successfully creates grant preview with custom parameters', async () => {
      ...
      await act(async () => {
        await result.current.handleGrantPermission([1], 'test prompt', customParams);
      });
    ...
    it('handles missing application address', async () => {
      ...
      expect(result.current.grantStatus).toContain('Application address not available');
      expect(result.current.isGranting).toBe(false);
    });
  ...
  describe('handleConfirmGrant', () => {
    ...
    it('handles grant creation errors', async () => {
      ...
      expect(result.current.grantStatus).toContain('Failed to grant permission');
      expect(result.current.isGranting).toBe(false);
  ...
  describe('handleRevokePermissionById', () => {
    ...
    it('handles revocation errors', async () => {
      ...
      expect(result.current.isRevoking).toBe(false);
      expect(consoleSpy).toHaveBeenCalledWith('Failed to revoke permission:', expect.any(Error));
  ...
  describe('handleLookupPermission', () => {
    ...
    it('successfully looks up permission and adds to list', async () => {
      mockVana.permissions.getPermissionInfo.mockResolvedValue(mockPermissionInfo);
      mockVana.permissions.getPermissionFileIds.mockResolvedValue([BigInt(4), BigInt(5)]);
      ...
      expect(mockVana.permissions.getPermissionInfo).toHaveBeenCalledWith(BigInt(3));
      expect(mockVana.permissions.getPermissionFileIds).toHaveBeenCalledWith(BigInt(3));
      expect(result.current.permissionLookupStatus).toContain('✅ Found permission: 3');
    ...
    it('handles lookup errors', async () => {
      const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
      mockVana.permissions.getPermissionInfo.mockRejectedValue(new Error('Permission not found'));
      ...
      expect(result.current.permissionLookupStatus).toContain('Failed to lookup permission');
      expect(result.current.isLookingUpPermission).toBe(false);
