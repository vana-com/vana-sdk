...
import { Schema, Refiner, AddSchemaParams, AddRefinerParams, UpdateSchemaIdParams } from '@opendatalabs/vana-sdk/browser';
...
describe('useSchemasAndRefiners', () => {
  ...
  describe('initialization', () => {
    it('returns default state when initialized', () => {
      ...
      expect(result.current.schemaDefinitionUrl).toBe('');
    ...
    it('loads schemas and refiners automatically when vana and address are available', async () => {
      ...
      expect(result.current.schemas[0]).toEqual({ ...mockSchemas[0], source: 'discovered' });
      ...
      expect(result.current.refiners[0]).toEqual({ ...mockRefiners[0], source: 'discovered' });
    });
  ...
  describe('loadSchemas', () => {
    ...
    it('handles individual schema loading failures gracefully', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Failed to load schema 2:', expect.any(Error));
  ...
  describe('loadRefiners', () => {
    ...
    it('handles errors when loading refiners fails', async () => {
      const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
      mockVana.data.getRefinersCount.mockRejectedValue(new Error('Network error'));
      ...
      expect(result.current.isLoadingRefiners).toBe(false);
      expect(consoleSpy).toHaveBeenCalledWith('Failed to load refiners:', expect.any(Error));
    ...
    it('handles individual refiner loading failures gracefully', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Failed to load refiner 2:', expect.any(Error));
  ...
  describe('handleCreateSchema', () => {
    it('successfully creates schema with valid inputs', async () => {
      mockVana.data.addSchema.mockResolvedValue({ schemaId: 3 });
      ...
      act(() => {
        ...
        result.current.setSchemaDefinitionUrl('https://newschema.com');
      });
      ...
      const expectedParams: AddSchemaParams = {
        name: 'New Schema',
        type: 'json',
        definitionUrl: 'https://newschema.com',
      };

      expect(mockVana.data.addSchema).toHaveBeenCalledWith(expectedParams);
      expect(result.current.schemaStatus).toContain('✅ Schema created with ID: 3');
    ...
    it('validates schema name is provided', async () => {
      ...
      act(() => {
        ...
        result.current.setSchemaType('json');
        result.current.setSchemaDefinitionUrl('https://schema.com');
      });
      ...
      expect(result.current.schemaStatus).toBe('❌ Please fill in all schema fields');
    ...
    it('validates schema definition URL is provided', async () => {
      ...
      act(() => {
        result.current.setSchemaName('Schema');
        result.current.setSchemaType('json');
        result.current.setSchemaDefinitionUrl('  ');
      });
  ...
  describe('handleCreateRefiner', () => {
    it('successfully creates refiner with valid inputs', async () => {
      ...
      expect(result.current.refinerStatus).toContain('✅ Refiner created with ID: 3');
    ...
    it('validates all refiner fields are provided', async () => {
      ...
      expect(result.current.refinerStatus).toBe('❌ Please fill in all refiner fields');
    ...
    it('validates DLP ID is a valid number', async () => {
      ...
      expect(result.current.refinerStatus).toBe('❌ DLP ID and Schema ID must be valid numbers');
      expect(mockVana.data.addRefiner).not.toHaveBeenCalled();
    });
  ...
  describe('handleUpdateSchemaId', () => {
    it('successfully updates schema ID with valid inputs', async () => {
      ...
      expect(result.current.updateSchemaStatus).toBe('✅ Schema ID updated successfully!');
    ...
    it('validates refiner ID is provided', async () => {
      ...
      expect(result.current.updateSchemaStatus).toBe('❌ Please provide both refiner ID and new schema ID');
    ...
    it('validates IDs are valid numbers', async () => {
      ...
      expect(result.current.updateSchemaStatus).toBe('❌ Both IDs must be valid numbers');
      expect(mockVana.data.updateSchemaId).not.toHaveBeenCalled();
    });

    it('handles update errors', async () => {
      const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
      mockVana.data.updateSchemaId.mockRejectedValue(new Error('Update failed'));
  ...
  describe('setters', () => {
    it('all setters update state correctly', () => {
      ...
      act(() => {
        result.current.setSchemaName('Test Schema');
        result.current.setSchemaType('json');
        result.current.setSchemaDefinitionUrl('https://test.com');
        result.current.setRefinerName('Test Refiner');
        result.current.setRefinerDlpId('1');
        result.current.setRefinerSchemaId('2');
        result.current.setRefinerInstructionUrl('https://instructions.com');
      ...
      expect(result.current.schemaDefinitionUrl).toBe('https://test.com');
      ...
      expect(result.current.refinerInstructionUrl).toBe('https://instructions.com');
