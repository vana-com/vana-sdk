...
export function useTrustedServers(): UseTrustedServersReturn {
  ...
  const [trustedServerQueryMode, setTrustedServerQueryMode] = useState<"subgraph" | "rpc" | "auto">("auto");
  ...
  const handleTrustServerGasless = useCallback(async (
  ...
  const handleUntrustServer = useCallback(async (serverIdToUntrust: string) => {
    if (!vana || !address) return;
    ...
    try {
      ...
      addToast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to untrust server",
        variant: "solid",
        color: "danger",
      });
    } finally {
  ...
  const handleDiscoverHostedServer = useCallback(async () => {
    ...
    try {
      ...
      if (!data.success) {
        throw new Error(data.error || "Gateway returned unsuccessful response");
      }
      
      const discoveredServerInfo = data.data;
      if (!discoveredServerInfo?.serverAddress || !discoveredServerInfo?.serverUrl) {
        throw new Error(
          "Invalid server discovery response: missing serverAddress or serverUrl"
        );
      }
