...
/**
 * Regenerates the complete index.ts file for all ABI exports
 *
 * @returns Promise that resolves when index file is regenerated
 */
async function updateIndexFile(): Promise<void> {
  ...
  const files = await fs.promises.readdir(abiDir);
  const implementationFiles = files
    .filter((f) => f.endsWith("Implementation.ts") && f !== "index.ts")
    .sort();

  // Read each file to get the actual export name
  const contracts: Array<{ name: string; abiName: string; fileName: string }> =
    [];

  for (const file of implementationFiles) {
    const contractName = file.replace("Implementation.ts", "");
    const filePath = path.join(abiDir, file);
    const content = await readFile(filePath, "utf-8");

    // Extract export name from "export const NameABI = [" pattern
    const exportMatch = content.match(/export const (\w+) = \[/);
    const abiName = exportMatch ? exportMatch[1] : `${contractName}ABI`;
  ...
  const imports = contracts
    .map((c) => `import { ${c.abiName} } from "./${c.fileName}";`)
    .join("\n");

  // Generate contractAbis object - only include contracts from CONTRACTS config that have ABIs
  const abiMappings: string[] = [];
  const allConfigContracts = { ...CONTRACTS, ...LEGACY_CONTRACTS };

  for (const contractName of Object.keys(allConfigContracts)) {
    const contract = contracts.find((c) => c.name === contractName);
    if (contract) {
      abiMappings.push(`  ${contractName}: ${contract.abiName},`);
    }
  }

  // Generate exports
  const exports = contracts.map((c) => c.abiName).join(",\n  ");
...
`;
  ...
  console.log(`✅ Generated ${indexPath} with ${contracts.length} contracts`);
}
...
async function fetchAndSaveABIs(
  ...
  // Regenerate index file with all ABIs
