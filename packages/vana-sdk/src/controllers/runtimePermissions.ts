// @ts-nocheck
// TODO: Remove @ts-nocheck when VanaRuntimePermissions contract is deployed and added to SDK
...
import { createRuntimeGrantFile, retrieveRuntimeGrantFile } from "../utils/runtimeGrantFiles";
...
export class RuntimePermissionsController extends BaseController {
  ...
  async createPermission(
    ...
    try {
      ...
      // 5. Get current block if startBlock not provided
      const startBlock =
        params.startBlock || (await this.context.publicClient.getBlockNumber());

      // 6. Call contract to create permission
      const account =
      ...
      const hash = await this.context.walletClient.writeContract({
        ...
        args: [params.datasetId, granteeId, grantUrl, startBlock, params.endBlock],
        account,
      ...
      // 8. Parse PermissionCreated event to get permission ID
  ...
  async isPermissionActive(permissionId: bigint): Promise<boolean> {
    ...
    return (await contract.read.isPermissionActive([
      permissionId,
    ])) as boolean;
  }
   ...
   *
   * @remarks
   * Returns an array of permission IDs that have been granted for the specified dataset.
   ...
   * @example
   * ```typescript
   * const permissionIds = await sdk.runtimePermissions.getDatasetPermissions(123n);
   * console.log(`Found ${permissionIds.length} permissions`);
   *
   * // Fetch details for each permission
   * for (const id of permissionIds) {
   *   const permission = await sdk.runtimePermissions.getPermission(id);
   *   console.log(`Permission ${id}: ${permission.grant}`);
   * }
   * ```
   */
  async getDatasetPermissions(datasetId: bigint): Promise<bigint[]> {
    const chainId = await this.context.publicClient.getChainId();
    const contractAddress = getContractAddress(
      chainId,
      "VanaRuntimePermissions",
