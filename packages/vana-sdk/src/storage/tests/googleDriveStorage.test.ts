...
import type { GoogleDriveConfig } from "../providers/google-drive";
...
describe("GoogleDriveStorage", () => {
  ...
  describe("Configuration", () => {
    ...
    it("should throw error when access token is missing", () => {
      expect(() => new GoogleDriveStorage({ accessToken: "" })).toThrow(
        StorageError,
      );
      expect(() => new GoogleDriveStorage({ accessToken: "" })).toThrow(
  ...
  describe("Upload", () => {
    ...
    it("should handle upload failure", async () => {
      ...
      await expect(storage.upload(testFile)).rejects.toThrow(
        "Failed to upload to Google Drive: Unauthorized: Invalid access token",
      );
    });
    ...
    it("should handle network errors during upload", async () => {
      ...
      await expect(storage.upload(testFile)).rejects.toThrow(StorageError);
      await expect(storage.upload(testFile)).rejects.toThrow(
        "Google Drive upload error: Network connection failed",
      );
    });
    ...
    it("should continue even if makeFilePublic fails", async () => {
      ...
      mockFetch
        ...
        .mockRejectedValueOnce(
          new Error("Network error during permissions request"),
        );
  ...
  describe("Download", () => {
    ...
    it("should handle download failures", async () => {
      ...
      await expect(
        storage.download("https://drive.google.com/file/d/missing-id/view"),
      ).rejects.toThrow(StorageError);
      await expect(
        storage.download("https://drive.google.com/file/d/missing-id/view"),
      ).rejects.toThrow("Failed to download from Google Drive: File not found");
    });
  ...
  describe("List", () => {
    it("should successfully list files from configured folder", async () => {
      ...
      expect(calledUrl).toContain(
        "q=trashed+%3D+false+and+%27test-folder-id%27+in+parents",
      );
      expect(calledUrl).toContain("pageSize=10");
      expect(calledUrl).toContain(
        "fields=files%28id%2Cname%2Csize%2CmimeType%2CcreatedTime%2CwebViewLink%29",
      );
    });
    ...
    it("should handle list API errors", async () => {
      ...
      await expect(storage.list()).rejects.toThrow(
        "Failed to list Google Drive files: Unauthorized",
      );
    });
    ...
    it("should list files without folderId filter", async () => {
      const configWithoutFolder = { accessToken: "test-token" };
      const storageWithoutFolder = new GoogleDriveStorage(configWithoutFolder);
      ...
      const files = await storageWithoutFolder.list();

      expect(files).toHaveLength(1);
      // Verify query doesn't include folderId
    ...
    it("should list files without namePattern filter", async () => {
      const mockResponse = {
        files: [
          {
            id: "file1",
      ...
      const files = await storage.list({ limit: 50 });

      expect(files).toHaveLength(1);
      // Verify query doesn't include namePattern
      const lastCall = mockFetch.mock.calls[mockFetch.mock.calls.length - 1];
      const url = new URL(lastCall[0]);
      const query = url.searchParams.get("q");
      expect(query).not.toContain("name contains");
    });

    it("should handle list with both folderId and namePattern", async () => {
      const mockResponse = {
        files: [
          {
            id: "file1",
      ...
      const files = await storage.list({ namePattern: "filtered", limit: 10 });

      expect(files).toHaveLength(1);
      // Verify query includes both folderId and namePattern
      const lastCall = mockFetch.mock.calls[mockFetch.mock.calls.length - 1];
      const url = new URL(lastCall[0]);
      const query = url.searchParams.get("q");
      expect(query).toContain("trashed = false");
      expect(query).toContain("'test-folder-id' in parents");
      expect(query).toContain("name contains 'filtered'");
    });
  });
  ...
  describe("Delete", () => {
    ...
    it("should handle 404 errors gracefully (already deleted)", async () => {
      ...
      const result = await storage.delete(
        "https://drive.google.com/file/d/missing-id/view",
      );
    ...
    it("should handle other delete errors", async () => {
      ...
      await expect(
        storage.delete("https://drive.google.com/file/d/test-id/view"),
      ).rejects.toThrow(StorageError);
      await expect(
        storage.delete("https://drive.google.com/file/d/test-id/view"),
      ).rejects.toThrow(
        "Failed to delete from Google Drive: Forbidden: Insufficient permissions",
      );
    });
    ...
    it("should extract file ID from various URL formats", async () => {
      const testCases = [
        {
          ...
          expected: "test-file-id-123",
        },
        {
          url: "https://drive.google.com/uc?id=test-file-id-456&export=download",
          expected: "test-file-id-456",
        },
      ...
      for (const testCase of testCases) {
        ...
        expect(lastCall[0]).toBe(
          `https://www.googleapis.com/drive/v3/files/${testCase.expected}?alt=media`,
        );
      }
    });
  ...
  describe("Token Refresh", () => {
    ...
    it("should throw error when refresh credentials are missing", async () => {
      ...
      const storageWithoutRefresh = new GoogleDriveStorage(
        configWithoutRefresh,
      );

      await expect(storageWithoutRefresh.refreshAccessToken()).rejects.toThrow(
        StorageError,
      );
      await expect(storageWithoutRefresh.refreshAccessToken()).rejects.toThrow(
    ...
    it("should handle token refresh failures", async () => {
      ...
      await expect(storage.refreshAccessToken()).rejects.toThrow(
        "Failed to refresh Google Drive token: Invalid refresh token",
      );
    });
    ...
    it("should handle network errors during token refresh", async () => {
      ...
      await expect(storage.refreshAccessToken()).rejects.toThrow(StorageError);
      await expect(storage.refreshAccessToken()).rejects.toThrow(
        "Google Drive token refresh error: Network timeout",
      );
    });
    ...
    it("should wrap unknown errors in StorageError", async () => {
      ...
      try {
        ...
        expect((error as StorageError).message).toContain(
          "Google Drive upload error: Something went wrong",
        );
