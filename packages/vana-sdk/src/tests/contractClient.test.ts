...
class TestContractClient extends ContractClient<"DataRegistry"> {
  ...
  public async testRead() {
    return (this.contract as any).read;
  }
}
...
describe("ContractClient", () => {
  ...
  describe("Construction", () => {
    ...
    it("should set up contract controller during construction", async () => {
      const { getContractController } = await import(
        "../contracts/contractController"
      );
      expect(getContractController).toHaveBeenCalledWith(
        "DataRegistry",
        provider.client,
      );
    });
  ...
  describe("getRawContract", () => {
    it("should return the contract instance", () => {
      ...
      expect(rawContract.address).toBe(
        "0x1234567890123456789012345678901234567890",
      );
      expect((rawContract as any).read).toBeDefined();
      expect((rawContract as any).write).toBeDefined();
    });
  ...
  describe("Multiple contract types", () => {
    it("should work with different contract types", async () => {
      class AnotherTestClient extends ContractClient<"PermissionRegistry"> {
        constructor(provider: VanaProvider) {
          super("PermissionRegistry", provider);
        }
      }
