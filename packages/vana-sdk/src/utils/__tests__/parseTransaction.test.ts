...
describe("parseTransaction", () => {
  ...
  it("returns empty expectedEvents when no registry exists for function", () => {
    const transactionResult = {
      hash: "0xtxhash" as `0x${string}`,
      from: "0xfrom" as `0x${string}`,
      contract: "UnknownContract" as TransactionResult["contract"],
      fn: "unknownFunction" as TransactionResult["fn"],
    };
    ...
    const result = parseTransaction(
      transactionResult as TransactionResult<never, never>,
      receipt,
    );
  ...
  it("handles malformed logs gracefully", () => {
    ...
    const receipt: TransactionReceipt = {
      logs: [
        ...
        {
          topics: null as unknown as Log["topics"], // Invalid topics
          data: "0x",
        } as unknown as Log,
        {
          // Missing data
