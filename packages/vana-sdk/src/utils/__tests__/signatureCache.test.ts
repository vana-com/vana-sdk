...
describe("SignatureCache", () => {
  ...
  describe("get", () => {
    it("should return null if no signature is cached", () => {
      const result = SignatureCache.get(mockCacheAdapter, walletAddress, messageHash);
  ...
  describe("set", () => {
    it("should store signature with default TTL", () => {
      SignatureCache.set(mockCacheAdapter, walletAddress, messageHash, signature);
      
      expect(mockCacheAdapter.set).toHaveBeenCalled();
      const [key, value] = mockCacheAdapter.set.mock.calls[0];
      expect(key).toBe(`vana_sig_${walletAddress.toLowerCase()}:${messageHash}`);
    ...
    it("should store signature with custom TTL", () => {
      ...
      SignatureCache.set(mockCacheAdapter, walletAddress, messageHash, signature, customTtlHours);
  ...
  describe("withSignatureCache", () => {
    it("should call sign function if no cached signature", async () => {
      ...
      const result = await withSignatureCache(mockCacheAdapter, walletAddress, typedData, signFn);
    ...
    it("should return cached signature without calling sign function", async () => {
      ...
      // Set up cache first
      await withSignatureCache(mockCacheAdapter, walletAddress, typedData, signFn);
      vi.clearAllMocks();
    ...
    it("should create different cache keys for different typed data", async () => {
      ...
      await withSignatureCache(mockCacheAdapter, walletAddress, typedData1, signFn);
      await withSignatureCache(mockCacheAdapter, walletAddress, typedData2, signFn);
    ...
    it("should handle BigInt values in typed data without throwing", async () => {
      ...
      const result = await withSignatureCache(mockCacheAdapter, walletAddress, typedDataWithBigInt, signFn);
    ...
    it("should create consistent cache keys for equivalent BigInt values", async () => {
      ...
      const result = await withSignatureCache(mockCacheAdapter, walletAddress, typedData2, signFn);
