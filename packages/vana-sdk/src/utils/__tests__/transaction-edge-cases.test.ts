...
describe("Transaction System Edge Cases", () => {
    ...
    it("handles logs with missing or malformed data", () => {
      ...
      const receipt = {
        logs: [
          ...
          { topics: null } as unknown as Log, // Null topics
          { topics: undefined } as unknown as Log, // Undefined topics
          { topics: [] } as unknown as Log, // Empty topics array
          { data: null } as unknown as Log, // Null data
  ...
  describe("Type safety edge cases", () => {
    ...
    it("prevents invalid contract/function combinations at runtime", () => {
      ...
      const parsed = parseTransaction(result, receipt);
  ...
  describe("Memory and performance edge cases", () => {
    it("creates independent objects (no shared references)", () => {
      ...
      (result1 as unknown as Record<string, unknown>).extra = "data";

      expect(
        (result2 as unknown as Record<string, unknown>).extra,
      ).toBeUndefined();
    });
  ...
  describe("Null and undefined edge cases", () => {
    it("does not accept null or undefined values", () => {
      ...
      expect(() => {
        tx({
          hash: null as unknown as `0x${string}`,
          from: "0xaddr" as `0x${string}`,
          contract: "DataRegistry",
          fn: "addFile",
        });
      ...
      expect(() => {
        tx({
          hash: "0xhash" as `0x${string}`,
          from: undefined as unknown as `0x${string}`,
          contract: "DataRegistry",
          fn: "addFile",
        });
      }).not.toThrow(); // tx() just passes through
    ...
    it("parseTransaction throws on null receipt", () => {
      ...
      expect(() => {
        parseTransaction(txResult, null as unknown as TransactionReceipt);
      }).toThrow();

      expect(() => {
        parseTransaction(txResult, undefined as unknown as TransactionReceipt);
      }).toThrow();
    });
  });
});
