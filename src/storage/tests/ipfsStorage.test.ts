...
describe('IPFSStorage', () => {
  ...
  describe('Configuration', () => {
    ...
    it('should throw error when API endpoint is missing', () => {
      ...
      expect(() => new IPFSStorage({ apiEndpoint: '' })).toThrow('IPFS API endpoint is required')
    })
  ...
  describe('Upload', () => {
    ...
    it('should use default gateway URL when not configured', async () => {
      ...
      expect(result.metadata?.gatewayUrl).toBe('https://gateway.pinata.cloud/ipfs/QmTestHash123')
    })
    ...
    it('should handle different hash field names in response', async () => {
      ...
      const testCases = [
        { response: { IpfsHash: 'QmPinataFormat' }, expected: 'QmPinataFormat' },
        { response: { Hash: 'QmStandardFormat' }, expected: 'QmStandardFormat' },
        { response: { hash: 'QmLowercaseFormat' }, expected: 'QmLowercaseFormat' }
      ]
    ...
    it('should handle upload failure with error response', async () => {
      ...
      await expect(storage.upload(testFile)).rejects.toThrow('Failed to upload to IPFS: Bad Request: File too large')
    })
    ...
    it('should handle missing hash in response', async () => {
      ...
      await expect(storage.upload(testFile)).rejects.toThrow(StorageError)
      await expect(storage.upload(testFile)).rejects.toThrow('IPFS upload succeeded but no hash returned')
    })
    ...
    it('should handle network errors during upload', async () => {
      ...
      await expect(storage.upload(testFile)).rejects.toThrow(StorageError)
      await expect(storage.upload(testFile)).rejects.toThrow('IPFS upload error: Network connection failed')
    })
  ...
  describe('Download', () => {
    it('should successfully download a file from IPFS URL', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://gateway.example.com/ipfs/QmTestHash123')
    })
    ...
    it('should successfully download from gateway URL', async () => {
      ...
      const result = await storage.download('https://ipfs.io/ipfs/QmTestHash123')
    ...
    it('should use default gateway when not configured', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://gateway.pinata.cloud/ipfs/QmTestHash123')
    })

    it('should handle invalid URL format', async () => {
      await expect(storage.download('https://example.com/invalid-url')).rejects.toThrow(StorageError)
      await expect(storage.download('https://example.com/invalid-url')).rejects.toThrow('Invalid IPFS URL format')
    })
    ...
    it('should handle download failures', async () => {
      ...
      await expect(storage.download('ipfs://QmMissingHash')).rejects.toThrow(StorageError)
      await expect(storage.download('ipfs://QmMissingHash')).rejects.toThrow('Failed to download from IPFS: Not Found')
    })

    it('should handle network errors during download', async () => {
      mockFetch.mockRejectedValue(new Error('Connection timeout'))

      await expect(storage.download('ipfs://QmTestHash')).rejects.toThrow(StorageError)
      await expect(storage.download('ipfs://QmTestHash')).rejects.toThrow('IPFS download error: Connection timeout')
    })
  })
  ...
  describe('List and Delete Operations', () => {
    it('should throw error for list operation (not supported)', async () => {
      ...
      await expect(storage.list()).rejects.toThrow('IPFS storage does not support file listing')
    })

    it('should throw error for delete operation (not supported)', async () => {
      await expect(storage.delete('ipfs://QmTestHash')).rejects.toThrow(StorageError)
      await expect(storage.delete('ipfs://QmTestHash')).rejects.toThrow('IPFS storage does not support file deletion')
    })
  ...
  describe('IPFS Hash Extraction', () => {
    it('should extract hash from various URL formats', async () => {
      const testCases = [
        ...
        { url: 'https://gateway.pinata.cloud/ipfs/QmTestHash123', expected: 'QmTestHash123' },
        { url: 'https://ipfs.io/ipfs/QmTestHash123', expected: 'QmTestHash123' },
        { url: 'QmTestHash123456789012345678901234567890123456', expected: 'QmTestHash123456789012345678901234567890123456' }
      ]
      ...
      for (const testCase of testCases) {
        ...
        expect(lastCall[0]).toBe(`https://gateway.example.com/ipfs/${testCase.expected}`)
      }
    })
    ...
    it('should handle invalid hash formats', async () => {
      ...
      for (const url of invalidUrls) {
        await expect(storage.download(url)).rejects.toThrow('Invalid IPFS URL format')
      }
    })
    ...
    it('should accept hash-only format for long enough strings', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(`https://gateway.example.com/ipfs/${longHash}`)
    })
  })

  describe('Error Handling', () => {
    it('should preserve StorageError instances', async () => {
      const originalError = new StorageError('Original error', 'TEST_CODE', 'ipfs')
    ...
    it('should wrap unknown errors in StorageError', async () => {
      ...
      try {
        ...
        expect(error.message).toContain('IPFS upload error: Something went wrong')
        expect(error.code).toBe('UPLOAD_ERROR')
        expect(error.provider).toBe('ipfs')
      }
    })
