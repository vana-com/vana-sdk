...
describe('PinataStorage', () => {
  ...
  describe('Configuration', () => {
    ...
    it('should throw error when JWT token is missing', () => {
      ...
      expect(() => new PinataStorage({ jwt: '' })).toThrow('Pinata JWT token is required')
    })
  ...
  describe('Upload', () => {
    ...
    it('should handle upload failure with error response', async () => {
      ...
      await expect(storage.upload(testFile)).rejects.toThrow('Pinata upload failed')
    })
    ...
    it('should handle missing IPFS hash in response', async () => {
      ...
      await expect(storage.upload(testFile)).rejects.toThrow(StorageError)
      await expect(storage.upload(testFile)).rejects.toThrow('no IPFS hash returned')
    })
    ...
    it('should handle network errors during upload', async () => {
      ...
      await expect(storage.upload(testFile)).rejects.toThrow(StorageError)
      await expect(storage.upload(testFile)).rejects.toThrow('Network connection failed')
    })
  ...
  describe('Download', () => {
    it('should successfully download a file from IPFS URL', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://test-gateway.pinata.cloud/ipfs/QmTestHash123')
    })
    ...
    it('should successfully download from gateway URL', async () => {
      ...
      const result = await storage.download('https://gateway.pinata.cloud/ipfs/QmTestHash123')
    ...
    it('should handle download failures', async () => {
      ...
      await expect(storage.download('ipfs://QmMissingHash')).rejects.toThrow('download from IPFS')
    })
    ...
    it('should handle network errors during download', async () => {
      ...
      await expect(storage.download('ipfs://QmTestHash')).rejects.toThrow('Connection timeout')
    })
  })
  ...
  describe('Delete', () => {
    ...
    it('should handle other delete errors', async () => {
      ...
      await expect(storage.delete('ipfs://QmTestHash')).rejects.toThrow('delete from Pinata')
    })
    ...
    it('should handle invalid URL format in delete', async () => {
      ...
      await expect(storage.delete('https://invalid-url.com')).rejects.toThrow('Invalid IPFS URL format')
    })
  })
  ...
  describe('IPFS Hash Extraction', () => {
    it('should extract hash from various URL formats', async () => {
      const testCases = [
        { url: 'ipfs://QmTestHash123456789012345678901234567890123456', expected: 'QmTestHash123456789012345678901234567890123456' },
        { url: 'https://gateway.pinata.cloud/ipfs/QmTestHash123456789012345678901234567890123456', expected: 'QmTestHash123456789012345678901234567890123456' }
      ]
      ...
      for (const testCase of testCases) {
        ...
        expect(lastCall[0]).toBe(`https://test-gateway.pinata.cloud/ipfs/${testCase.expected}`)
      }
    })
  ...
  describe('Error Handling', () => {
    it('should preserve StorageError instances', async () => {
      const originalError = new StorageError('Original error', 'TEST_CODE', 'pinata')
    ...
    it('should wrap unknown errors in StorageError', async () => {
      ...
      try {
        ...
        expect(error.message).toContain('Pinata upload error: Something went wrong')
