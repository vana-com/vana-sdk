...
import { StorageProvider, StorageUploadResult, StorageFile, StorageError } from '../index'
...
describe('StorageManager', () => {
  ...
  describe('Provider Retrieval', () => {
    ...
    it('should throw error when no provider specified and no default set', () => {
      ...
      expect(() => emptyManager.getProvider()).toThrow('No storage provider specified and no default provider set')
    })

    it('should throw error when provider not found', () => {
      expect(() => storageManager.getProvider('nonexistent')).toThrow(StorageError)
      expect(() => storageManager.getProvider('nonexistent')).toThrow("Storage provider 'nonexistent' not found")
    })
  })
  ...
  describe('Default Provider Management', () => {
    ...
    it('should throw error when setting non-existent provider as default', () => {
      expect(() => storageManager.setDefaultProvider('nonexistent')).toThrow(StorageError)
  ...
  describe('Storage Operations', () => {
    ...
    describe('upload', () => {
      it('should upload using default provider', async () => {
        ...
        expect(mockProvider1.uploadMock).toHaveBeenCalledWith(mockBlob, 'test.txt')
      ...
      it('should upload using specified provider', async () => {
        ...
        const result = await storageManager.upload(mockBlob, 'test.txt', 'provider2')
        
        expect(mockProvider2.uploadMock).toHaveBeenCalledWith(mockBlob, 'test.txt')
      ...
      it('should handle upload errors properly', async () => {
        const mockBlob = new Blob(['test data'], { type: 'text/plain' })
        const mockError = new StorageError('Upload failed', 'UPLOAD_ERROR', 'provider1')
        
        mockProvider1.uploadMock.mockRejectedValue(mockError)
        
        await expect(storageManager.upload(mockBlob, 'test.txt')).rejects.toThrow(mockError)
      })
    })
    ...
    describe('download', () => {
      it('should download using default provider', async () => {
        ...
        const result = await storageManager.download('https://example.com/file.txt')
        
        expect(mockProvider1.downloadMock).toHaveBeenCalledWith('https://example.com/file.txt')
      ...
      it('should download using specified provider', async () => {
        ...
        const result = await storageManager.download('https://example.com/file.txt', 'provider2')
        
        expect(mockProvider2.downloadMock).toHaveBeenCalledWith('https://example.com/file.txt')
        expect(mockProvider1.downloadMock).not.toHaveBeenCalled()
        expect(result).toBe(mockBlob)
      })

      it('should handle download errors properly', async () => {
        const mockError = new StorageError('Download failed', 'DOWNLOAD_ERROR', 'provider1')
        mockProvider1.downloadMock.mockRejectedValue(mockError)
        
        await expect(storageManager.download('https://example.com/file.txt')).rejects.toThrow(mockError)
      })
    })
    ...
    describe('list', () => {
      ...
      it('should handle list errors properly', async () => {
        const mockError = new StorageError('List failed', 'LIST_ERROR', 'provider1')
    ...
    describe('delete', () => {
      it('should delete using default provider', async () => {
        ...
        const result = await storageManager.delete('https://example.com/file.txt')
        
        expect(mockProvider1.deleteMock).toHaveBeenCalledWith('https://example.com/file.txt')
      ...
      it('should delete using specified provider', async () => {
        ...
        const result = await storageManager.delete('https://example.com/file.txt', 'provider2')
        
        expect(mockProvider2.deleteMock).toHaveBeenCalledWith('https://example.com/file.txt')
        expect(mockProvider1.deleteMock).not.toHaveBeenCalled()
        expect(result).toBe(false)
      })

      it('should handle delete errors properly', async () => {
        const mockError = new StorageError('Delete failed', 'DELETE_ERROR', 'provider1')
        mockProvider1.deleteMock.mockRejectedValue(mockError)
        
        await expect(storageManager.delete('https://example.com/file.txt')).rejects.toThrow(mockError)
      })
    })
  })
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle null/undefined parameters appropriately', async () => {
      ...
      expect(mockProvider1.uploadMock).toHaveBeenCalledWith(mockBlob, undefined)
