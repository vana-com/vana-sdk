...
describe('Encryption Utils', () => {
  ...
  describe('DEFAULT_ENCRYPTION_SEED', () => {
    it('should export the standard Vana encryption seed', () => {
      expect(DEFAULT_ENCRYPTION_SEED).toBe('Please sign to retrieve your encryption key')
    })
  })
  ...
  describe('generateEncryptionKey', () => {
    ...
    it('should throw error if wallet has no account', async () => {
      ...
      await expect(generateEncryptionKey(walletWithoutAccount))
        .rejects.toThrow('Wallet account is required for encryption key generation')
    })

    it('should handle signing errors', async () => {
      mockWallet.signMessage.mockRejectedValue(new Error('Signing failed'))

      await expect(generateEncryptionKey(mockWallet))
  ...
  describe('Parameter validation', () => {
    it('should validate wallet parameter', async () => {
      await expect(generateEncryptionKey(null as any))
        .rejects.toThrow()
    })
    ...
    it('should validate wallet account property', async () => {
      ...
      await expect(generateEncryptionKey(invalidWallet as any))
  ...
  describe('Error message formatting', () => {
    it('should preserve original error messages', async () => {
      const originalError = new Error('Specific wallet error: insufficient funds')
  ...
  describe('encryptUserData', () => {
    ...
    it('should encrypt user data successfully', async () => {
      const testData = new Blob(['sensitive user data'], { type: 'text/plain' })
    ...
    it('should handle createMessage errors', async () => {
      ...
      mockOpenpgp.createMessage.mockRejectedValue(new Error('Invalid message format'))

      await expect(encryptUserData(testData, encryptionKey))
        .rejects.toThrow('Failed to encrypt user data: Invalid message format')
    })
    ...
    it('should handle encryption errors', async () => {
      ...
      await expect(encryptUserData(testData, encryptionKey))
        .rejects.toThrow('Failed to encrypt user data: Encryption failed')
    })
    ...
    it('should handle Response processing errors', async () => {
      ...
      global.Response = vi.fn().mockImplementation(() => ({
        arrayBuffer: () => Promise.reject(new Error('Response processing failed'))
      })) as any

      await expect(encryptUserData(testData, encryptionKey))
        .rejects.toThrow('Failed to encrypt user data: Response processing failed')
    })
  })
  ...
  describe('decryptUserData', () => {
    ...
    it('should decrypt user data successfully', async () => {
      ...
      const mockDecryptedData = new Uint8Array([100, 101, 99, 114, 121, 112, 116, 101, 100]) // 'decrypted'
    ...
    it('should handle readMessage errors', async () => {
      ...
      mockOpenpgp.readMessage.mockRejectedValue(new Error('Invalid encrypted message'))

      await expect(decryptUserData(encryptedData, encryptionKey))
        .rejects.toThrow('Failed to decrypt user data: Invalid encrypted message')
    })
    ...
    it('should handle wrong encryption key', async () => {
      ...
      const mockMessage = { data: 'encrypted-message' }
      mockOpenpgp.readMessage.mockResolvedValue(mockMessage)
      mockOpenpgp.decrypt.mockRejectedValue(new Error('Session key decryption failed'))

      await expect(decryptUserData(encryptedData, wrongKey))
        .rejects.toThrow('Failed to decrypt user data: Session key decryption failed')
    })
  })
  ...
  describe('Encrypt-Decrypt Integration', () => {
    ...
    it('should handle full encrypt-decrypt workflow', async () => {
      ...
      // Mock decryption
      const mockDecryptedData = new Uint8Array([99, 111, 110, 102, 105, 100, 101, 110, 116, 105, 97, 108])
      mockOpenpgp.readMessage.mockResolvedValue(mockMessage)
      mockOpenpgp.decrypt.mockResolvedValue({ data: mockDecryptedData })
